...
# Reuse existing nodebuilder-solidinvoice container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-solidinvoice; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-solidinvoice -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
